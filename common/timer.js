...
var timer = {
    ...
    second2hour: function (value, type = 0) {
        ...
        var dayTime = 0;
        if (secondTime > 0) {
          dayTime = Math.floor(secondTime / (60 * 60 * 24));
          hourTime = Math.floor(secondTime / (60 * 60)) - (dayTime * 24);
          minuteTime = Math.floor(secondTime / 60) - (hourTime * 60) - (dayTime * 60 * 24);
          secondTime = Math.floor(secondTime) - (hourTime * 60 * 60) - (minuteTime * 60) - (dayTime * 60 * 60 * 24);
        }
      if (dayTime < 10) {
        dayTime = `0${dayTime}`
      }
        ...
        if (type == 0) {
          var result = `${dayTime}`+'天'+`${hourTime}:${minuteTime}:${secondTime}`;
            return result;
        } else {
            return {
				dayTime: dayTime,
