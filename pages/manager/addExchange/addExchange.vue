<template>
	<view class="wrap">
		<!-- 调休时间 -->
		<view class="top-area">选择调休时间:</view>
		<!-- 主体 -->
		<view class="main-area">
						...
						<block v-for="(item, index) in dateArr">
							<view>
								...
								<block v-else-if="item.date_time > today">
									<view class="calendar-date future" @click="clickDate" :data-year="year" :data-month="month" :data-date="item.dateNum" :data-index="index">
										<view :class="[(date_one == item.date_time || date_two == item.date_time)?'now':'']">{{item.dateNum}}</view>
										<view class="holiday" v-if="item.is_work == 2">休假</view>
									</view>
								</block>
							</view>
						</block>
			...
			<view class="content-area">
				<textarea class="content" @input="getContent" placeholder="调休理由..." placeholder-style="color:#999;font-size:13px;"></textarea>
			</view>
			<!-- 提交 -->
			<view class="btn-area">
	...
	export default {
		...
		methods: {
			...
			clickDate:function(e){
				...
				if(that.date_one=='' || that.date_one==null){
					if(dateArr[index].is_work==1){
						that.date_one = date;
					}else{
						uni.showToast({
							title:'请选择调休日期',
			...
			submit:function(){
				...
				if(that.date_two=='' || that.date_two==null){
					uni.showToast({
						title:'请选择休息日期',
						icon:'none',
						duration:2000
				...
				var data = {
					date_one: that.date_one,
					date_two:that.date_two,
					content: that.content,
					manager_id: userInfo.manager_id
				};
				tools.httpClient("home/WxStore/addManagerExchange",data,function(err,res){
					if(res.errorCode==0){
						uni.showToast({
							title:'提交成功,等待后台审核!',
							icon:'success',
