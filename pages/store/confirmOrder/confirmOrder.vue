<template>
	<view class="wrap">
		...
		<view class="inner-content">
			<view class="head-top">
				<view class="back" @click="back"><text class="iconfont icon-zuojiantou"></text></view>
			...
			<view class="address-area">
				<view class="address-list" v-if="addressInfo" @click="selectAddress">
					<view class="address-info">
						...
						<view class="address-bot" v-if="addressInfo.identify_num">身份号码：{{addressInfo.identify_num}}</view>
						<view class="address-bot" v-else>身份号码未填写</view>
					</view>
				</view>
			...
			<view class="good-area">
				<block v-for="(item,index) in goodsInfo">
					<view class="good-info">
						<image :src="base_url+item.img_url" class="good-img"></image>
						<view class="good">
							<view class="good-top">
								<view class="name">{{item.simple_desc}}</view>
							</view>
							<view class="good-mid" v-if="item.property_text">
								<view class="name">{{item.property_text}}</view>
							</view>
							<view class="good-bot">
			...
			<view class="remark">
				...
				<textarea class="remark_input" v-model="remark" type="text" maxlength="100"></textarea>
			</view>
			<!-- 支付方式 -->
			...
			<view class="order-area">
				<view class="order-info">
					...
					<view class="pay-type-right" @click="changeCheck">
						<text class="iconfont icon-weixuanzhongyuanquan" v-if="is_check == 0"></text>
	...
	export default {
		...
		methods: {
			getConfirmInfo: function(gids,gpids,counts){
				var that = this;
				tools.httpClient("home/WxStore/getConfirmInfo",{'gids':gids,'gpids':gpids,'counts':counts},function(err,res){
					if(res.errorCode == 0){
			...
			getUserInfo:function(){
				var that = this;
				tools.httpClient('home/WxStore/getUserInfo',{'user_id':that.userInfo.id}, (error, res) => {
					if (res.errorCode == 0) {
			...
			getAddressInfo:function(){
				var that = this;
				tools.httpClient('home/WxStore/getAddress',{'user_id':that.userInfo.id}, (error, res) => {
					if (res.errorCode == 0) {
			...
			getConfigInfo:function(){
				var that = this;
				tools.httpClient('home/WxStore/getConfigInfo',{'gids':this.gids,'gpids':this.gpids,'counts':this.counts}, (error, res) => {
					if (res.errorCode == 0) {
