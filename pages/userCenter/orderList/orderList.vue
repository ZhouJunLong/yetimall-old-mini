<template>
	<view class="wrap">
		...
		<view class="order-area">
			<!-- 订单状态 -->
			<view class="tab-area" v-if="language == 0">
				<view :class="[state == -1?'tab-list cur':'tab-list']" data-state="-1" @click="changeState">온통</view>
				<view :class="[state == 0?'tab-list cur':'tab-list']" data-state="0" @click="changeState">지불 대기</view>
				<view :class="[state == 1?'tab-list cur':'tab-list']" data-state="1" @click="changeState">발송 대기 화물</view>
				<view :class="[state == 2?'tab-list cur':'tab-list']" data-state="2" @click="changeState">받 을 물건</view>
				<view :class="[state == 3?'tab-list cur':'tab-list']" data-state="3" @click="changeState">완료 됨</view>
				<view :class="[state == 7?'tab-list cur':'tab-list']" data-state="7" @click="changeState">환불 됨</view>
			</view>
			<view class="tab-area" v-else>
				<view :class="[state == -1?'tab-list cur':'tab-list']" data-state="-1" @click="changeState">全部</view>
				<view :class="[state == 0?'tab-list cur':'tab-list']" data-state="0" @click="changeState">待付款</view>
				<view :class="[state == 1?'tab-list cur':'tab-list']" data-state="1" @click="changeState">待发货</view>
				<view :class="[state == 2?'tab-list cur':'tab-list']" data-state="2" @click="changeState">待收货</view>
				<view :class="[state == 3?'tab-list cur':'tab-list']" data-state="3" @click="changeState">已完成</view>
				<view :class="[state == 7?'tab-list cur':'tab-list']" data-state="7" @click="changeState">已退款</view>
			</view>
			<!-- 订单列表 -->
					...
					<view class="order-list">
						<view class="order-top">
							<view class="order-top-left">
								<view class="order-num" v-if="language == 2">주문 번호：{{item.order_num}}</view>
								<view class="order-num" v-else>订单编号：{{item.order_num}}</view>
								<image 
								class="copy"
						...
						<block v-for="(items, indexs) in item.goods_list">
							<view class="order-mid">
								<view class="good-list" @click="gotoOrderDetail" :data-gid="items.gid" :data-oid="item.id">
									<view class="good-img">
										<image mode="widthFix" :src="base_url+items.thumb"></image>
									</view>
						...
						<view class="order-bot">
							<view class="order-desc">
								...
								<text class="unit" v-if="language == 0">합계 하 다：</text>
								<text class="unit" v-else>合计：</text>
						...
						<view class="btn-area">
							...
							<view :class="item.btn_opt" v-if="item.btn_show" @click="operationOrder" :data-token="item.waybill_token" :data-operation="item.btn_opt" :data-index="index">{{item.btn_text}}</view>
							<view :class="item.btn_opt2" v-if="item.btn_show2" @click="operationOrder" :data-operation="item.btn_opt2" :data-index="index">{{item.btn_text2}}</view>
						</view>
					</view>
				</block>
			</block>
			<block v-else>
				<view class="no-more" v-if="language == 0">주문 없 음</view>
				...
				for (var i = 0; i < dataList.length; i++) {
					...
					if(goods_list[0]['property']){
						...
						for(var k = 0; k<property.length;k++){
							property_text += property[k]['name'] +':'+property[k]['value']+' ';
						}
	...
	export default {
		...
		methods: {
			...
			operationOrder:function(e){
				...
				if(operation == 'payOrder'){//立即支付
					that.payOrder(index);
				}else if(operation == 'refundOrder'){//申请退款
					that.refundOrder(index);
				}else if(operation == 'addComment'){//评价
					that.addComment(index);
				}else if(operation == 'deleteOrder'){//删除
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							if(is_click){
								is_click = false;
								tools.httpClient("home/WxStore/setOrderState",data,function(err,res){
									uni.hideLoading();
									if(res.errorCode==0){
										is_click = true;
										//重新加载分页信息
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							tools.httpClient("home/WxStore/deleteOrder",data,function(err,res){
								uni.hideLoading();
								if(res.errorCode==0){
									//重新加载分页信息
									page = 1;
